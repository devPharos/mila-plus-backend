...
class PaymentCriteriaController {
    ...
    async update(req, res) {
        ...
        try {
            ...
            if (!criteriaExists) {
                return res.status(401).json({ error: 'Payment criteria does not exist.' });
            }

            await criteriaExists.update({ ...req.body, updated_by: req.userId, updated_at: new Date() }, {
                transaction: t
            });
            await t.commit();
    ...
    async delete(req, res) {
        ...
        try {
            ...
            return res.status(200).json({ message: 'Payment criteria deleted successfully.' });
        } catch (err) {
            await t.rollback();
            const className = 'PaymentCriteriaController';
